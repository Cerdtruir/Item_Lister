  ...
  def perform
    ...
    CSV.open('public/woocommerce_items.csv', 'w', write_headers: true, headers:) do |writer|
      items.each do |item|
        ...
        writer << [item.id, 'simple', "#{item.name} - #{item.condition}", 1, 0, 'visible', "#{item.description} - #{item.condition}", "#{item.description} - #{item.condition}",
                   'taxable', in_stock, item.quantity, 0, 0, 1, item.selling_price, 'Products', "https://res.cloudinary.com/dwi7jdore/image/upload/#{item.id}.jpg"]
      end
    end
  end
end
